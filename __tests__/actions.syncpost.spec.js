...
describe('test syncPost function', () => {
  it('syncPosts no error', async done => {
    nock('https://api.tvmaze.com/').get(`/shows/${id}`).reply(200, {});
  ...
  it('syncPosts error 500', async done => {
    nock('https://api.tvmaze.com/').get(`/shows/${id}`).replyWithError({ message: 'Error server', code: '500' });
  ...
  it('syncPosts error 404', async done => {
    nock('https://api.tvmaze.com/').get(`/shows/${id}`).reply(404);
